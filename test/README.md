...
Each suite loads `index.html` into jsdom, drives the real interface, and asserts on what actually happens. There is no mocking of the page itself.

| Suite | Covers |
|---|---|
| `1-core` | Page boots without errors, tabs, timeline, map selection, playbook, action tracker, letter generation end to end |
...
The site is one file with a lot of interdependent copy. The tone pass alone rewrote about forty strings, and a stray apostrophe in one of them broke the whole page silently — the tests caught it in seconds.
